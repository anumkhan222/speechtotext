...
# Runs blocking Whisper/LLM calls outside the event loop
...
async def load_models():
    ...
    def _warmup():
        try:
            ...
            print(
                f"[startup] Ollama warmup failed — "
...
async def stream_audio(
    ...
    frame_bytes_len = (
        vad._frame_samples * config.BYTES_PER_SAMPLE
    )
    ...
    async def run_correction():
        ...
        if corrected:
            await db.update_corrected_text(
                session_id,
                corrected,
            )
    ...
    async def schedule_silence_correction():
        try:
            await asyncio.sleep(
                config.CORRECT_ON_SILENCE_MS / 1000
            )
    ...
    async def handle_segment(segment: dict):
        ...
        looks_like_hallucination = (
            ...
            and (
                ...
                or segment["duration_ms"]
                < config.MIN_TRUSTED_SEGMENT_MS
            )
        )
    ...
    try:
        ...
        while True:
            ...
            # Re-chunk arbitrary-sized incoming audio
            # into fixed VAD frames
            while len(audio_buf) >= frame_bytes_len:

                frame = bytes(
                    audio_buf[:frame_bytes_len]
                )
